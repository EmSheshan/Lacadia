...
function generateMainPokemonImage ( pokemon, outputDir ) {
    ...
    const bgPath = path.join (__dirname, 'assets', 'IMG_0952.png');
    ...
    loadImage (imagePath).then (( image ) => {
        ...
        Promise.all (typeIconPromises).then (( [type1Image, type2Image] ) => {
            ...
            ctx.drawImage (image, 100, 100, 800, 800); // Center the image and adjust the size as needed
...
function renderAbilities(ctx, abilityList) {
    const filteredAbilities = abilityList.filter(ability => ability && ability.trim() !== '');
    ...
    filteredAbilities.forEach((ability, index) => {
        const currentBoxY = boxY + index * (boxHeight + spacing);
        ...
        // Check if ability is an object with tag 'lacadia'
        if (typeof abilities[ability] === 'object' && abilities[ability].tag === 'lacadia') {
            description = abilities[ability].description; // use 'description' property
            color = '#00aaff';
        } else {
        ...
        const textY = currentBoxY + boxHeight / 2 - 30;
        ctx.fillText(`${abilityName}`, textX, textY);

        ctx.font = '20px Ubuntu';
        renderTextBlock(ctx, description, textX - 17, textY + 20, boxWidth, 20);
    });
}
